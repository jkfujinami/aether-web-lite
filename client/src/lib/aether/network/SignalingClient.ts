...
import type { PeerId, SignalingMessage } from '../types';
...
export class SignalingClient {
  ...
  public async connect(options: SignalingOptions): Promise<void> {
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'peers') {
            ...
            // トラッカーを中継点とした WebRTC ネゴシエーション
            // sender(targetPeerId) からのシグナリングデータ
            if (this.onRelayCb) this.onRelayCb(data.senderId, data);
